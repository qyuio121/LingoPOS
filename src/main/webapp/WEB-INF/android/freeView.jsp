...
var showComments = function(key){		
	$.ajax({
		url:"<c:url value='/Android/Comment.Lingo'/>",
		data:{freeno:key},
		dataType:'json',
		type:'post',
...
var displayComments	 = function(data){
	...
	$('.commentDelete').click(function(){			
		...
		$.ajax({
			url:"<c:url value='/Android/CommentDelete.Lingo'/>",
...
$(function(){
	...
	$('#submit').click(function(){				
		$.ajax({
			url:"<c:url value='/Android/CommentWrite.Lingo'/>",
			data:$('#frm').serialize(),
			dataType:'text',
			type:'post',
			success:function(key){					
	...
	$('#del_memo').click(function(){
		if(confirm("정말 삭제 하시겠습니까")){
			location.replace("<c:url value='/Android/FreeDelete.Lingo?freeno=${record.freeno}'/>");
		}
	});
});
</script>
...
<!-- 바디 헤더 시작-->
	<div class="row">
		<div class="col-xs-12">
			<h2 ><img src="<c:url value='/Images/apple.png'/>" alt="image" style="width: 40px" />자유게시판</h2>
		</div>
	</div>	
<!-- 바디 헤더 끝-->
<!-- 상세보기 내용 예시 시작  -->
	...
	<div class="row">
		<div class="text-center">
			<c:if test="${sessionScope.loginDTO.id==record.id }">
				<a href="<c:url value='/Android/FreeEdit.Lingo?freeno=${record.freeno}'/>" class="btn btn-primary">수정</a>
				<a id="del_memo" href="#" class="btn btn-primary">삭제</a>
			</c:if>
			<a href="<c:url value='/Android/Free.Lingo?searchWord=${param.searchWord}&searchColumn=${param.searchColumn}&nowPage=${param.nowPage}'/>" class="btn btn-primary">목록</a>
		</div>
	</div>	
